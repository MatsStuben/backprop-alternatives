...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        if epoch % 20 == 0 or epoch == epochs - 1:
            ...
            print(f"Epoch {epoch:4d}, 3F: {train_loss_three_factor:.6f}, 2F: {train_loss_two_factor:.6f}, BP: {train_loss_backprop:.6f}, 2F-M: {train_loss_momentum:.6f}, NP: {train_loss_node_perturb:.6f}")
    with torch.no_grad():
    ...
    print(f"Test MSE on grid: {test_mse_three_factor:.6f}, {test_mse_two_factor:.6f}, {test_mse_backprop:.6f}, {test_mse_momentum:.6f}, {test_mse_node_perturb:.6f}")
    ...
    plt.xlabel('x')
    plt.ylabel('y')
    plt.title(f'Prediction vs True (MSE={test_mse_three_factor:.4f}, {test_mse_two_factor:.4f}, {test_mse_backprop:.4f}, {test_mse_momentum:.4f}, {test_mse_node_perturb:.4f})')
    plt.legend()
    plt.tight_layout()
    plt.show()
    print("Done")