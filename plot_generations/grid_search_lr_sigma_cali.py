...
def run_single_trial(method, X_train, y_train, eta, sigma, epochs, batch_size, seed=None):
    ...
    dimensions = (X_train.shape[1], 128, 64, 1)
...
def main():
    ...
    ETA_GRID = [0.005, 0.01, 0.03, 0.1]
    SIGMA_GRID = [0.03, 0.1, 0.2, 0.5]

    X_train, y_train, _, _ = load_cali_data(seed=None)
    ...
    plot_table(speeds, f"Convergence speed (updates) – {METHOD} / cali")
    plot_table(levels, f"Convergence level (median last 10%) – {METHOD} / cali")
    plot_table(variances, f"Variance (last 10%) – {METHOD} / cali")
